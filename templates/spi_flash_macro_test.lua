...
function SpiFlashMacroTest:_init(tLogger)
  ...
  self.tLogger:debug('Using vstruct V%s', self.vstruct._VERSION)
...
function SpiFlashMacroTest:spi_configuration_set_speed_khz(atSpiConfiguration, ulSpeedFifoKhz, ulSpeedSqiRomKhz)
  ...
  if ulSpeedFifoKhz<=0 then
    self.tLogger:error('The speed for the FIFO mode must be larger than 0. Here it is %s.', tostring(ulSpeedFifoKhz))
    tResult = nil
  end
  if ulSpeedSqiRomKhz<0 then
    self.tLogger:error('The speed for the SQI ROM mode must be larger than or equal to 0. Here it is %s.', tostring(ulSpeedSqiRomKhz))
    tResult = nil
  end
...
function SpiFlashMacroTest:spi_configuration_set_port_control(atSpiConfiguration, usCLK, usMOSI, usMISO, usSIO2, usSIO3, usCS0)
  ...
  if usCLK<0 or usCLK>0xffff then
    self.tLogger:error('The port control value for CLK must be an integer in the range [0, 65535]. Here it is %d.', tostring(usCLK))
    tResult = nil
  end
  if usMOSI<0 or usMOSI>0xffff then
    self.tLogger:error('The port control value for MOSI must be an integer in the range [0, 65535]. Here it is %d.', tostring(usMOSI))
    tResult = nil
  end
  if usMISO<0 or usMISO>0xffff then
    self.tLogger:error('The port control value for MISO must be an integer in the range [0, 65535]. Here it is %d.', tostring(usMISO))
    tResult = nil
  end
  if usSIO2<0 or usSIO2>0xffff then
    self.tLogger:error('The port control value for SIO2 must be an integer in the range [0, 65535]. Here it is %d.', tostring(usSIO2))
    tResult = nil
  end
  if usSIO3<0 or usSIO3>0xffff then
    self.tLogger:error('The port control value for SIO3 must be an integer in the range [0, 65535]. Here it is %d.', tostring(usSIO3))
    tResult = nil
  end
  if usCS0<0 or usCS0>0xffff then
    self.tLogger:error('The port control value for CS0 must be an integer in the range [0, 65535]. Here it is %d.', tostring(usCS0))
    tResult = nil
  end
...
function SpiFlashMacroTest:spi_configuration_set_mmio(atSpiConfiguration, ucCS, ucCLK, ucMISO, ucMOSI, ucSIO2, ucSIO3)
  ...
  if ucCS<0 or ucCS>0xff then
    self.tLogger:error('The MMIO index for CS must be an integer in the range [0, 255]. Here it is %d.', tostring(ucCS))
    tResult = nil
  end
  if ucCLK<0 or ucCLK>0xff then
    self.tLogger:error('The MMIO index for CLK must be an integer in the range [0, 255]. Here it is %d.', tostring(ucCLK))
    tResult = nil
  end
  if ucMISO<0 or ucMISO>0xff then
    self.tLogger:error('The MMIO index for MISO must be an integer in the range [0, 255]. Here it is %d.', tostring(ucMISO))
    tResult = nil
  end
  if ucMOSI<0 or ucMOSI>0xff then
    self.tLogger:error('The MMIO index for MOSI must be an integer in the range [0, 255]. Here it is %d.', tostring(ucMOSI))
    tResult = nil
  end
  if ucSIO2<0 or ucSIO2>0xff then
    self.tLogger:error('The MMIO index for ucSIO2 must be an integer in the range [0, 255]. Here it is %d.', tostring(ucSIO2))
    tResult = nil
  end
  if ucSIO3<0 or ucSIO3>0xff then
    self.tLogger:error('The MMIO index for ucSIO3 must be an integer in the range [0, 255]. Here it is %d.', tostring(ucSIO3))
    tResult = nil
  end
...
function SpiFlashMacroTest:spi_configuration_set_dummy_byte(atSpiConfiguration, ucDummyByte)
  ...
  if ucDummyByte<0 or ucDummyByte>0xff then
    self.tLogger:error('The dummy byte must be an integer in the range [0, 255]. Here it is %d.', tostring(ucDummyByte))
    tResult = nil
  end
...
function SpiFlashMacroTest:spi_configuration_set_mode(atSpiConfiguration, ucMode)
  ...
  if tResult~=true then
    self.tLogger:error('Invalid mode specified: %s. Accepted values are %s.', tostring(ucMode), table.concat(atValid, ', '))
  else
...
function SpiFlashMacroTest:spi_configuration_set_idle(atSpiConfiguration, ucIdleConfiguration)
  ...
  if ucIdleConfiguration<0 or ucIdleConfiguration>0xff then
    self.tLogger:error('The idle configuration must be an integer in the range [0, 255]. Here it is %d.', tostring(ucIdleConfiguration))
    tResult = nil
  end
    ...
    if string.sub(strStrippedLine, 1, 1)~='#' then
      ...
      for _, strToken in pairs(astrTokens) do
        ...
        if ucNumber~=nil then
          ...
          self.tLogger:debug('[SPI Macro] %02x: 0x%02x', ulCurrentAddress, ucNumber)
          ...
          if string.sub(strStrippedToken, -1)==':' then
            ...
            if atLabels[strLabelName]~=nil then
              ...
              self.tLogger:debug('[SPI Macro] Label "%s" created at address 0x%02x', strLabelName, ulCurrentAddress)
            end
            ...
            if tToken~=nil then
              ...
              self.tLogger:debug('[SPI Macro] %02x: 0x%02x (%s)', ulCurrentAddress, tToken, strStrippedToken)
              ulCurrentAddress = ulCurrentAddress + 1
            else
              ...
              self.tLogger:debug('[SPI Macro] Added reference to label "%s" at address 0x%02x.', strLabelName, ulCurrentAddress)
              ulCurrentAddress = ulCurrentAddress + 1
            end
          end
        end
...
function SpiFlashMacroTest:compile(atSpiConfiguration, uiUnit, uiChipSelect, strSpiMacro)
  ...
  if strType~='number' then
    self.tLogger:error('The unit must be a number. Here it has the type %s.', strType)
    tResult = nil
  elseif uiUnit<0 then
    self.tLogger:error('The unit must be larger than or equal to 0. Here it is %d.', uiUnit)
    tResult = nil
  end

  strType = type(uiChipSelect)
  if strType~='number' then
    self.tLogger:error('The chip select must be a number. Here it has the type %s.', strType)
    tResult = nil
  elseif uiChipSelect<0 then
    self.tLogger:error('The chip select must be larger than or equal to 0.')
    tResult = nil
  end

  if string.len(strSpiMacro)>self.SPI_MACRO_MAX_SIZE then
    self.tLogger:error('The SPI macro is too long.')
    fAllOK = false
  end
